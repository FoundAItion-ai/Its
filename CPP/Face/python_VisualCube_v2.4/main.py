...
import math
import random
from typing import List, Tuple, Dict, Type, Sequence, Any, Optional
...
class SimAgent:
    ...
    def __init__(self, cls_name: str, pos: Tuple[float, float],
                 ...
                 *,
                 ...
                 base_turn_deg_dt: float = cfg.AGENT_DEFAULT_TURN_RATE_DEG_PER_DT,
        ...
        # Consolidate all parameters for the controller.
        # This includes specific params like thresholds, random frequencies, etc.
        ...
        controller_params.update({
            ...
            "initial_base_turn_rate_deg_dt": base_turn_deg_dt
        })
        ...
        self.body = AgentKinematics(
            ...
            max_turn_rate_deg_per_dt=base_turn_deg_dt
        )
    ...
    def _create_controller(self, cls_name: str, params: Dict[str, Any]) -> _BaseAgent | composite_agent:
        ...
        # Special handling for composite agent with no layers
        if cls_name == "composite" and (not params.get("layer_configs")):
             ...
             params["layer_configs"] = [{
                 ...
                 "agent_params": {} # Use defaults for the layer
             }]
        ...
        try:
            ...
            return stochastic_motion_random_movement_agent(
                ...
                output_time_scale=agent_module_AGENT_CONFIG["OUTPUT_TIME_SCALE_FALLBACK"],
                initial_base_turn_rate_deg_dt=params.get("initial_base_turn_rate_deg_dt", cfg.AGENT_DEFAULT_TURN_RATE_DEG_PER_DT)
            )
...
def run_frame(is_visual: bool = True):
    if WORLD:
        WORLD.step()
        if is_visual and WORLD.surface is not None: WORLD.draw(WORLD.surface)
...
def get_simulation_stats() -> dict:
    ...
    return {k: "N/A" for k in ["total_food", "active_agents", "food_eaten_run", "total_frames_simulated_run", "current_frame_dt"]}
