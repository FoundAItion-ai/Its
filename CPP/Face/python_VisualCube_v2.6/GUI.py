...
class SimGUI:
    def __init__(self):
        ...
        self._tk_vars = {
            ...
            # New 4-layer params for inverse agent
            "inv_l1_left_hz": VarD(2.0),
            "inv_l1_right_hz": VarD(2.0),
            "inv_l2_left_hz": VarD(10.0),
            "inv_l2_right_hz": VarD(10.0),
        }

        # New vars for trace and logging
        self.draw_trace_var = tk.BooleanVar(value=False)
        ...
        self.screenshot_requested_path: Optional[str] = None
    ...
    def _build_controls_tab(self, parent_tab):
        ...
        self._add_slider_entry(motion_frame, self._tk_vars["agent_constant_speed"], "Speed-Frequency Scale\n(px/s per oar Hz)", 10, 300, res=1.0, width=28)
        ...
        f = ttk.Frame(world_frame); f.pack(fill="x", pady=2)
        ttk.Checkbutton(f, text="Draw Agent Trace (Breadcrumbs)", variable=self.draw_trace_var).pack(side="left", padx=5)
        ttk.Checkbutton(f, text="Enable Frame-by-Frame Logging (.log)", variable=self.enable_logging_var).pack(side="left", padx=5)
        ...
        self.screenshot_button = ttk.Button(btns, text="Save Screenshot", command=self.request_screenshot, state="disabled")
        self.screenshot_button.pack(side="left", expand=True, fill="x", padx=5)
    ...
    def _on_agent_type_change(self, *args):
        ...
        if self.specific_agent_params_frame:
            ...
            if selected_type == "inverse_turn":
                self._add_slider_entry(self.specific_agent_params_frame, self._tk_vars["inv_l1_left_hz"], "L1 Left Freq (Hz)", 0.1, 20.0, res=0.1)
                self._add_slider_entry(self.specific_agent_params_frame, self._tk_vars["inv_l1_right_hz"], "L1 Right Freq (Hz)", 0.1, 20.0, res=0.1)
                self._add_slider_entry(self.specific_agent_params_frame, self._tk_vars["inv_l2_left_hz"], "L2 Left Freq (Hz)", 0.1, 20.0, res=0.1)
                self._add_slider_entry(self.specific_agent_params_frame, self._tk_vars["inv_l2_right_hz"], "L2 Right Freq (Hz)", 0.1, 20.0, res=0.1)
                self.specific_agent_params_frame.pack(fill="x", pady=(5,0), after=self.agent_sel.master.master)
            elif selected_type == "stochastic":
                self._add_slider_entry(self.specific_agent_params_frame, self._tk_vars["stoch_left_rand_freq"], "Left Rand. Freq (prob/decision)", 0.0, 1.0, res=0.01)
                self._add_slider_entry(self.specific_agent_params_frame, self._tk_vars["stoch_right_rand_freq"], "Right Rand. Freq (prob/decision)", 0.0, 1.0, res=0.01)
                self.specific_agent_params_frame.pack(fill="x", pady=(5,0), after=self.agent_sel.master.master)
            else:
    ...
    def _build_status_tab(self, parent_tab):
        ...
        # New status label for the inverse agent threshold
        self.inv_thresh_var = tk.StringVar(value="Calibrated Threshold: N/A"); ttk.Label(stat, textvariable=self.inv_thresh_var).pack(anchor="w", pady=2)
    ...
    def _periodic_status_update(self):
        ...
        if self.running_simulation and not self.headless_run_active:
            ...
            # Update inverse agent threshold display
            if stats.get('agent_type') == 'inverse_turn':
                if stats.get('inv_agent_calibrated', False):
                    thresh_hz = stats.get('inv_agent_threshold_hz', 0.0)
                    fps = self._tk_vars["fps"].get()
                    if fps > 0:
                        thresh_per_30f = (thresh_hz / fps) * 30
                        self.inv_thresh_var.set(f"Calibrated Threshold: {thresh_per_30f:.2f} food/30-frames")
                    else:
                        self.inv_thresh_var.set("Calibrated Threshold: Invalid FPS")
                else:
                    self.inv_thresh_var.set("Calibrated Threshold: N/A (awaiting 2nd food)")
            else:
                 self.inv_thresh_var.set("Calibrated Threshold: N/A")
    ...
    def _apply_settings(self, for_headless=False) -> bool:
        ...
        try:
            ...
            params = self._get_main_agent_specific_params(headless=for_headless)
            ...
            main_configure(
                ...
                # New parameters for trace and logging
                draw_trace=self.draw_trace_var.get(),
    ...
    def start_simulation(self):
        ...
        if self.enable_logging_var.get():
            ...
            filename = f"raw_output_{agent_type}_{food_preset}_{timestamp}.log"
            try:
                self.active_log_file = open(filename, 'w')
                self._add_log_message(f"Frame logging enabled, writing to {filename}")
                self._write_log_header()
            except IOError as e:
                self._add_log_message(f"Error opening log file: {e}")
                self.active_log_file = None
    ...
    def stop_simulation(self):
        ...
        if self.active_log_file:
            self._add_log_message(f"Closing log file: {self.active_log_file.name}")
            self.active_log_file.close()
            self.active_log_file = None
    ...
    def _simulation_loop_thread(self):
        ...
        while self.running_simulation:
            ...
            if self.screenshot_requested_path:
                try:
                    pygame.image.save(self.pygame_surface, self.screenshot_requested_path)
                    path = self.screenshot_requested_path
                    self.root.after(0, lambda p=path: self._add_log_message(f"Screenshot saved to {p}"))
                except pygame.error as e:
                    path = self.screenshot_requested_path
                    self.root.after(0, lambda p=path, err=e: self._add_log_message(f"Failed to save screenshot to {p}: {err}"))
                finally:
                    self.screenshot_requested_path = None
    ...
    def _headless_run_worker_thread(self):
        try:
            ...
            for i in range(num_instances):
                ...
                self._apply_settings(for_headless=True)
    ...
    def _get_main_agent_specific_params(self, headless=False) -> Dict[str, Any]:
         ...
         if agent_type == "inverse_turn":
             params['l1_left_hz'] = self._tk_vars["inv_l1_left_hz"].get()
             params['l1_right_hz'] = self._tk_vars["inv_l1_right_hz"].get()
             params['l2_left_hz'] = self._tk_vars["inv_l2_left_hz"].get()
             params['l2_right_hz'] = self._tk_vars["inv_l2_right_hz"].get()
    ...
    def request_screenshot(self):
        """Opens a file dialog to save the current simulation screen."""
        if not self.running_simulation or not self.pygame_surface:
            messagebox.showwarning("Not Running", "Cannot take a screenshot because the visual simulation is not running.")
            return
        ...
        filepath = filedialog.asksaveasfilename(
            ...
            initialfile=f"screenshot_{time.strftime('%Y%m%d-%H%M%S')}.png"
        )
        if filepath:
            self.screenshot_requested_path = filepath

    def _write_log_header(self):
        """Writes a detailed configuration header to the active log file."""
        if not self.active_log_file:
            return
        
        f = self.active_log_file
        f.write(f"# Simulation Log - {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"# --- CONFIGURATION ---\n")
        
        agent_type = self.agent_type_var.get()
        f.write(f"# Agent Type: {agent_type}\n")
        f.write(f"# Food Preset: {self.food_sel.get()}\n")
        
        params = {
            "num_agents": self._tk_vars["n_agents"].get(),
            "fps": self._tk_vars["fps"].get(),
            "spawn_point": (self._tk_vars["spawn_x"].get(), self._tk_vars["spawn_y"].get()),
            "initial_orientation_deg": self._tk_vars["initial_orientation_deg"].get(),
            "speed_freq_scale": self._tk_vars["agent_constant_speed"].get(),
            "output_time_scale_frames": self._tk_vars["output_time_scale"].get(),
        }
        ...
        f.write(f"# --- DATA ---\n")
        f.write(f"# frame,agent_id,pos_x,pos_y,angle_rad\n")
