...
from config import set_agent_speed_scaling_factor
from agent_module import set_k_theta_deg, K_THETA_DEG as agent_k_theta_deg
...
class SimGUI:
    def __init__(self):
        ...
        self._tk_vars = {
            ...
            "k_theta_deg": VarD(agent_k_theta_deg),
        ...
        self.composite_scrollable_frame: Optional[ttk.Frame] = None
    ...
    def _build_controls_tab(self, parent_tab):
        ...
        self._add_slider_entry(physics_frame, self._tk_vars["agent_speed_scaling_factor"], "Speed Scaling Factor", 1, 500, res=1.0, w=28)
        self._add_slider_entry(physics_frame, self._tk_vars["k_theta_deg"], "Rotation Factor", 1, 180, res=1.0, w=28)
    ...
    def _on_agent_type_change(self, *args):
        ...
        if self.specific_agent_params_frame:
            ...
            if sel_type == "stochastic":
                self._add_slider_entry(self.specific_agent_params_frame, self._tk_vars["stoch_update_interval_sec"], "Re-randomize Interval (s)", 0.5, 10.0, 0.1)
                self.specific_agent_params_frame.pack(fill="x", pady=(5,0), after=self.agent_sel.master.master)
            elif sel_type == "inverse_turn":
                thresh_frame = ttk.LabelFrame(self.specific_agent_params_frame, text="Thresholds"); thresh_frame.pack(fill='x', padx=5, pady=3)
                self._add_slider_entry(thresh_frame, self._tk_vars["inv_threshold_r"], "Right Threshold (Hz)", 0.1, 20.0, 0.1)
                self._add_slider_entry(thresh_frame, self._tk_vars["inv_threshold_l"], "Left Threshold (Hz)", 0.1, 20.0, 0.1)
    ...
    def _rebuild_composite_gui(self):
        ...
        for i, pair_vars in enumerate(self.composite_layer_pair_vars):
            ...
            pair_frame.pack(fill="x", padx=10, pady=5, expand=True)
    ...
    def _remove_composite_layer_pair(self, vars_to_remove):
        if len(self.composite_layer_pair_vars) > 1:
            self.composite_layer_pair_vars.remove(vars_to_remove)
            self._rebuild_composite_gui()
        else:
            messagebox.showwarning("Cannot Remove", "The composite agent must have at least one layer pair.")
    ...
    def _periodic_status_update(self):
        ...
        if self.running_simulation and not self.headless_run_active:
            ...
            if agent_type in ['inverse_turn', 'composite']:
                freq = stats.get('input_food_frequency', 0.0)
                self.agent_input_freq_var.set(f"Input Freq (Hz): {freq:.2f}")
            else:
    ...
    def _apply_settings(self, for_headless=False) -> bool:
        ...
        try:
            ...
            cfg.set_global_speed_modifier(self._tk_vars["global_speed_modifier"].get())
            set_agent_speed_scaling_factor(self._tk_vars["agent_speed_scaling_factor"].get())
            set_k_theta_deg(self._tk_vars["k_theta_deg"].get())
            ...
            main_configure(
                ...
                draw_trace=self.draw_trace_var.get(), log_file_handle=self.active_log_file
            )
    ...
    def start_simulation(self):
        ...
        if self.enable_logging_var.get():
            ...
            fname = f"raw_output_{self.agent_type_var.get()}_{self.food_sel.get()}_{ts}.log"
            try:
                self.active_log_file = open(fname, 'w'); self._add_log_message(f"Logging to {fname}"); self._write_log_header()
    ...
    def stop_simulation(self):
        ...
        if self.active_log_file and not self.active_log_file.closed:
            ...
            self.active_log_file.write(f"# FINAL_STATS: food_eaten={food_eaten}\n")
    ...
    def _simulation_loop_thread(self):
        ...
        try:
            ...
            pygame.display.set_caption("Agent Sim"); self.root.after(0, lambda: self.sim_status_var.set("Simulation: Running"))
        ...
        while self.running_simulation:
            for event in pygame.event.get():
                if event.type == pygame.QUIT: self.root.after(0, self.stop_simulation); break 
    ...
    def _headless_run_worker_thread(self):
        try:
            ...
            for i in range(num_instances):
                if not self.headless_run_active:
                    self.root.after(0, lambda: self._add_log_message("Headless run aborted."))
                    break
    ...
    def _write_log_header(self):
        ...
        f.write(f"# Food Preset: {food_preset}\n")
        try:
            initial_food_count = len(cfg.FOOD_PRESETS[food_preset]())
            f.write(f"# initial_food_count: {initial_food_count}\n")
        except Exception:
            f.write(f"# initial_food_count: 0\n")

        params = {
            "num_agents": self._tk_vars["n_agents"].get(), "fps": self._tk_vars["fps"].get(),
            "global_speed_modifier": self._tk_vars["global_speed_modifier"].get(),
            "agent_speed_scaling_factor": self._tk_vars["agent_speed_scaling_factor"].get(),
            "k_theta_deg": self._tk_vars["k_theta_deg"].get(),
            "spawn_point": (self._tk_vars["spawn_x"].get(), self._tk_vars["spawn_y"].get()),
            "initial_direction_deg": self._tk_vars["initial_direction_deg"].get(),
        }
    ...
    def _analyze_log_file(self):
        ...
        try:
            ...
            fps_match = re.search(r'# fps: (\d+)', "".join(header))
            food_eaten_match = re.search(r'# FINAL_STATS: food_eaten=(\d+)', "".join(header))
            if not fps_match or not food_eaten_match: raise ValueError("Log file missing required stats.")
            fps = int(fps_match.group(1)); food_eaten = int(food_eaten_match.group(1))
            total_distance = 0.0; last_frame = 0
            for line in data:
                parts = line.strip().split(',')
                if len(parts) < 7: continue
                last_frame = int(parts[0])
                if int(parts[1]) == 0: total_distance += float(parts[5])
            total_time_sec = last_frame / fps if fps > 0 else 0
            food_per_sec = food_eaten / total_time_sec if total_time_sec > 0 else 0
            dist_per_sec = total_distance / total_time_sec if total_time_sec > 0 else 0
            food_per_1k_pixels = (food_eaten / total_distance) * 1000 if total_distance > 0 else 0
            result_str = (
                f"Log Analysis: {filepath.split('/')[-1]}\n"
                f"------------------------------------\n"
                f"Total Time: {total_time_sec:.2f} seconds\n"
                f"Total Food Eaten: {food_eaten}\n"
                f"Total Distance Traveled: {total_distance:.2f} pixels\n"
                f"------------------------------------\n"
                f"Efficiency (Food/Sec): {food_per_sec:.3f}\n"
                f"Path Efficiency (Food/1000px): {food_per_1k_pixels:.3f}\n"
                f"Average Speed (px/sec): {dist_per_sec:.2f}\n"
            )
            messagebox.showinfo("Log Analysis Results", result_str)
        except Exception as e:
            messagebox.showerror("Analysis Error", f"Could not parse the log file.\nError: {e}")
